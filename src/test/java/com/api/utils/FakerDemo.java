package com.api.utils;

import java.util.Locale;

import com.github.javafaker.Faker;

public class FakerDemo {

	public static void main(String[] args) {
		// We will using faker libray togeenarte fake test data
		//We willbe creating Faker Utility that willuse this faker library
		Locale locale= new Locale("en-India");
		Faker faker = new Faker(locale);
		String firstName= faker.name().firstName();
		System.out.println(firstName);
		String lastName= faker.name().lastName();
		System.out.println(lastName);
		String address= faker.address().buildingNumber();
		System.out.println(address);
		
		System.out.println(faker.address().streetAddress());
		System.out.println(faker.address().streetName());
		System.out.println(faker.address().city());
		System.out.println(faker.number().digits(10));
		System.out.println(faker.numerify("9986######"));
		System.out.println(faker.internet().emailAddress());
		
		System.out.println(faker.numerify("9986######"));
		System.out.println(faker.phoneNumber().cellPhone());
		
		
		
		

	}

}
